...
exports.updateCourse = asyncHandler(async (req, res, next) => {
	...
	// Make sure course is exists and logged in user is course owner
	if (!course) return next(new ErrorResponse(`Course not found with id of ${req.params.id}`, 404));
	if (course.user.toString() !== req.user.id && req.user.role !== "admin") return next(new ErrorResponse(`User ${req.user.id} is not authorized to update this course`, 401));
...
exports.deleteCourse = asyncHandler(async (req, res, next) => {
	...
	// Make sure course is exists and logged in user is course owner
	if (!course) return next(new ErrorResponse(`Course not found with id of ${req.params.id}`, 404));
	if (course.user.toString() !== req.user.id && req.user.role !== "admin") return next(new ErrorResponse(`User ${req.user.id} is not authorized to delete this course`, 401));
