...
exports.updateReview = asyncHandler(async (req, res, next) => {
	...
	if (!review) return next(new ErrorResponse(`Review not found with id of ${req.params.id}`, 404));
	if (review.user.toString() !== req.user.id && req.user.role !== "admin") return next(new ErrorResponse(`User ${req.user.id} is not authorized to update a review`, 401));
...
exports.deleteReview = asyncHandler(async (req, res, next) => {
	...
	if (!review) return next(new ErrorResponse(`Review not found with id of ${req.params.id}`, 404));
	if (review.user.toString() !== req.user.id && req.user.role !== "admin") return next(new ErrorResponse(`User ${req.user.id} is not authorized to delete a review`, 401));
